function [ZPX2,MIP,MI] = NMSA_to_nb2_ZPX2(nmsa)
% This function differs from nbZPX2 as we use the partial correlation
% matrix between sequences as the distance matrix.

[nrows,ncols] = size(nmsa);
dist = NMSA_to_RHO(nmsa);
...
%--------------------------------------------------------------------------
% Here we reorder based on the 2nd algorithm modified to use the rho matrix
% instead of the standard distance matrix.
ordered = reorder_nmsa_2_rho(nmsa,nrows,ncols);
%--------------------------------------------------------------------------
...
function [ordered] = reorder_nmsa_2_rho(nmsa,nrows,ncols)
%--------------------------------------------------------------------------
dist = NMSA_to_RHO(nmsa);
% Here we are using the partial correlation matrix derived from the
% covariance matrix. The diagonal is already the smallest number (=-1); 
...
for i = 2:nrows
    % set to -1 the entire test column; therefore that position will not be
    % considered any more.
    dist(:,ind) = -1;
    % find the index of the closest sequence
    [~,newind] = max(dist(ind,:));
