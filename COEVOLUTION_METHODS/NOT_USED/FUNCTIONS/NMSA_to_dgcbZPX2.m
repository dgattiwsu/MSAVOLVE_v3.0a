function [ZPX2] = NMSA_to_dgcbZPX2(nmsa)
...
% [sMI] = BNMSA_to_sMI(bin_ordered,bin_ordered_ind,nrows,ncols);
% [bMI] = BNMSA_to_pIJ(bin_ordered,bin_ordered_ind,nrows,ncols);
[pIJ] = BNMSA_to_pIJ(bin_ordered,bin_ordered_ind,nrows,ncols);
% [bMI] = cov(bin_ordered).*sMI;
[bMI] = cov(bin_ordered).*pIJ;
...
%    pH_2 = pH;
    COV_2 = COV;
%--------------------------------------------------------------------------
% End the 2nd reordering algorithm
%--------------------------------------------------------------------------
...
function [sMI] = BNMSA_to_sMI(bnmsa,bin_ordered_ind,nrows,ncols)
%--------------------------------------------------------------------------
% MI calculation; the following are the rules used in generating the
% arrays.
% pI(1) == 1 ; pI(2) == 0 
...
%         epIJ(4) = pI(2,I)*pI(2,J);
%         ind = find(pIJ);
%         ind = intersect(ind,[1 4]);
%         if pIJ(1) == 0
%         else
%         pMI(I,J) = (sum(pIJ(ind) .* (log2(pIJ(ind) ./ epIJ(ind)) + const)))/nrows;
%         pMI(I,J) = (pIJ(1) .* (log2(pIJ(1) ./ epIJ(1)) + const))/nrows;
%         end
        sMI(I,J) = (bnmsa(:,I)'*bnmsa(:,J))/nrows;
    end
end
sMI = sMI+sMI';
for m = 1:ncols
