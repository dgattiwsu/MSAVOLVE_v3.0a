function [ZPX2] = NMSA_to_fbhZPX2(nmsa)
% In this function we reorder the msa such that there is the minimum 
% distance between each sequence and the following one. The top sequence is 
% the sequence that is closer to the consensus sequence. 
...
REF_nmsa = nmsa;
REF_smsa = int2aa(nmsa);

[REF_profile_gaps,REF_symbols_gaps] = seqprofile(REF_smsa,'Gaps',gaps_count);
% REF_bg_frequencies_gaps = sum(REF_profile_gaps,2)/npos;

% nmsa_gap_ind = REF_nmsa == 25;
% REF_nmsa(nmsa_gap_ind) = 21;
% [REF_rel_entropy_aa_gaps,REF_rel_entropy_gaps] = rel_entropy_nmsa_gaps(REF_nmsa,...
%     REF_bg_frequencies_gaps,REF_profile_gaps);

nmsa_rel_entropy = nmsa;
nmsa_gap_ind = nmsa == 25;
nmsa(nmsa_gap_ind) = 21;
for i = 1:nrows
    for j = 1:ncols
%        nmsa_rel_entropy(i,j) = REF_rel_entropy_aa_gaps(nmsa(i,j),j);
         nmsa_rel_entropy(i,j) = REF_profile_gaps(nmsa(i,j),j);
    end
end

nmsa_rel_entropy_sum = sum(nmsa_rel_entropy,2);
[sorted_nmsa_rel_entropy_sum,sorted_ind] = ...
    sort(nmsa_rel_entropy_sum,'descend');
    
nmsa = nmsa(sorted_ind,:);
% 
%--------------------------------------------------------------------------
