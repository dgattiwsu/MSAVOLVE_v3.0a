function [ZPX2] = NMSA_to_ndcbZPX2(nmsa)
...
z_ordered(~bin_ordered) = 0;
    ...
    % Here we calculate the pIJ matrix that will multiply the COV matrix
    [pIJ] = BNMSA_to_pIJ(bin_ordered,bin_ordered_ind,nrows,ncols);
    % MI matrix
    [MI_1] = NMSA_to_MI(z_ordered);
    COV_1 = cov(bin_ordered).*pIJ;
    
%--------------------------------------------------------------------------
% We redo everything again for the other type of ordering.    
...
z_ordered(~bin_ordered) = 0;
    ...
    [pIJ] = BNMSA_to_pIJ(bin_ordered,bin_ordered_ind,nrows,ncols);
    % MI matrix
    [MI_2] = NMSA_to_MI(z_ordered);
    COV_2 = cov(bin_ordered).*pIJ;

% Here we merge everything.
%    pH = (pH_1 + pH_2)/2;
    MI = MI_1 + MI_2;
    COV = COV_1 + COV_2;
