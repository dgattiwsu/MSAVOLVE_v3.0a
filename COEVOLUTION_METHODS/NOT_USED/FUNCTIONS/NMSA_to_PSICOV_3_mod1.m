function [pcZPX2,pcMIP,ppvZPX2,ppvMIP,gcov_inv,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_PSICOV_3_mod1(nmsa,threshold,psc_lambda,...
...
% submatrices of the global covariance matrix. 
...
% NMSA_to_PSICOV_3(nmsa,0.62,1,0.015,0.5,0.001,'QUIC','INVERSE',20);
% To reproduce PSICOV use: [pcZPX2,pcMIP,ppvZPX2,ppvMIP] =
% NMSA_to_PSICOV_3(nmsa,0.62,1,0.015,0.5,0.001,'GLASSO','INVERSE',20);
    ...
    for oi = 1:ncols
        ...
        for oj = oi:ncols
            ...
            switch recov_method
                case 'FRO'
                % Frobenius norm
                subcov = gcov_inv(i:i+shift,j:j+shift);
                for k = 1:nsymbols
                    subcov(k,k) = 0;
                end
                cov_inv(oi,oj) = norm(subcov,'fro');
