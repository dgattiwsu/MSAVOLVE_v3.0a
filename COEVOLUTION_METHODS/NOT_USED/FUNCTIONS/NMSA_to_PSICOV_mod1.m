function [pcZPX2,pcMIP,ppvZPX2,ppvMIP,gCOV,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_PSICOV_mod1(nmsa,threshold,psc_lambda,...
    Lmat,lambda,delta,inverse_method,recov_method,pc_method,nsymbols)
% _mod1 differs from the regular one as we zero the diagonals of the
% submatrices of the global covariance matrix. Also we use the correlation
% matrix instead of the covariance matrix as a distance matrix. This
% function is the 1st of three alternative implementations of the PSICOV
% algorithm. It is identical to NMSA_to_PSICOV_1 and is slightly faster
% than _2, but gives more or less the same result. It calculates a partial
% correlation matrix for the columns of an MSA. The MSA is first converted
% to its binary 'long' representation in which every column is extended
% into 21 different columns, each one representing a different aa. A
...
% standard representation. The value of nsymbols determines whether we
% include or not gaps in the calculation of the 'shrunk' covariance matrix
% (nsymbols = 21 includes gaps; nsymbols = 20 excludes gaps). Finally, an
