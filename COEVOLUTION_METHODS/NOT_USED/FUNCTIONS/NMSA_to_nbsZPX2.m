function [ZPX2,ZPX4] = NMSA_to_nbsZPX2(nmsa)
% This function differs from NMSA_to_nbZPX2, as the original product of ij 
% zscore is replaced with their sum [ZPX2] or the sum of their squares [XPX4].
...
z_ordered(~bin_ordered) = 0;
    ...
    % and finally sZPX2/sZPX4 matrix
    [ZPX2,ZPX4] = MIP_to_sZPX2(MIP,ncols);
...
function [pZPX2,pZPX4] = MIP_to_sZPX2(pMIP,ncols)
%--------------------------------------------------------------------------
% ZPX2 calculation
mean_row=zeros(ncols,1);
std_row=zeros(ncols,1);
pZPX2=zeros(ncols,ncols);
pZPX4=zeros(ncols,ncols);
...
for m=1:ncols
    ...
    % Here we substitute a sum for the product of the original ZPX2
    
    pZPX2(m,n)=(ZPX2_i)+(ZPX2_j);
    pZPX4(m,n)=(ZPX2_i^2)+(ZPX2_j^2);

% No need to correct any longer for the presence of negative values.
