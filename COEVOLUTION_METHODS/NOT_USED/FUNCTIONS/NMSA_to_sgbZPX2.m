function [ZPX2] = NMSA_to_sgbZPX2(nmsa)
...
% ordered = reorder_nmsa_2(nmsa,nrows,ncols);
...
bin_ordered = sparse(bin_ordered);    
bin_ordered_sum = sum(bin_ordered);
bin_ordered_ind = find(bin_ordered_sum);

% Here we calculate the MI matrix and mean column entropy only for the 
% indices included in the vector 'bin_ordered_in'.

    [gMI,pH] = BNMSA_to_MI(bin_ordered,bin_ordered_ind,nrows,ncols);
    gMI = full(gMI);
    
    % Here we recover the original values of nrows and ncols

    [~,oncols] = size(nmsa);
    MI = zeros(oncols,oncols);
    for oi = 1:oncols
        i = 21*(oi-1) + 1;
        for oj = oi:oncols
            j = 21*(oj-1)+1;
            MI(oi,oj) = norm(gMI(i:i+20,j:j+20),'fro');
            MI(oj,oi) = MI(oi,oj);
        end
    end
    
    % MIP matrix
    MIP = MI_to_MIP(MI,oncols);

    % and finally ZPX2 matrix
    ZPX2 = MIP_to_ZPX2(MIP,oncols);
