function [pcZPX2,pcMIP,ppvZPX2,ppvMIP,cov_inv,rho,COV,sCOV,lambda,nonzeros] = ...
    NMSA_to_PSICOV_2(nmsa,threshold,psc_lambda,...
    Lmat,lambda,delta,inverse_method,pc_method,nsymbols)
% This function is the 2nd of three alternative implementations of the
% PSICOV algorithm. It very similar to NMSA_to_PSICOV_1, but is slightly
% slower and perhaps a little more accurate. The MSA is first converted to
% its binary 'long' representation in which every column is extended into
% 21 different columns, each one representing a different aa. A threshold
% is applied for the similarity between sequences. A pseudocount
% (psc_lambda) is also applied. It is at this point that it differs from
% _1, as also the columns that contains all zeros are included in the
% pseudocount. Then, the covariance matrix is calculated and is shrunk to
% the equivalent covariance matrix for the 'short' standard representation.
% The value of nsymbols determines whether we include or not gaps in the
% calculation of the 'shrunk' covariance matrix (nsymbols = 21 includes
% gaps; nsymbols = 20 excludes gaps). Finally, an inverse method is used to
% invert the covariance matrix. Three option are available for this
...
% same dimensions as the covariance matrix to be inverted. 'pc_method'
...
% NMSA_to_PSICOV_2(nmsa,0.62,1,0.005,0.0,0.0001,'QUIC','RHO',20);
% To reproduce PSICOV use: [pcZPX2,pcMIP,ppvZPX2,ppvMIP] =
% NMSA_to_PSICOV_2(nmsa,0.62,1,0.015,0.0,0.0001,'GLASSO','INVERSE',20);
...
% vector 'bin_ordered_ind'.
    ...
    nonzeros = nnz(cov_inv)/(ncols*ncols);
    fprintf('Sparsity = %f \n', nonzeros*100);
