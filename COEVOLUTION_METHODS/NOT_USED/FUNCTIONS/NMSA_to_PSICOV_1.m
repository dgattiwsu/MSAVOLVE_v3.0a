function [pcZPX2,pcMIP,ppvZPX2,ppvMIP,gCOV,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_PSICOV_1(nmsa,dist_method,threshold,psc_lambda,...
...
% extended into 20 (dist_method = 'NOGAPS) or 21 (dist_method = 'GAPS)
% different columns, each one representing a different aa. A threshold is
% applied for the similarity between sequences. A pseudocount (psc_lambda)
% is also applied. Then, the covariance matrix is calculated and is shrunk
% to the equivalent covariance matrix for the 'short' standard
% representation. The value of nsymbols determines whether we include or
% not gaps in the calculation of the 'shrunk' covariance matrix (nsymbols =
% 21 includes gaps; nsymbols = 20 excludes gaps). Finally, an inverse
...
% matrix to be inverted. 'pc_method' determines whether we calculate the
...
% by the application of a logistic fit to the MIP data. Possible usage:
% [pcZPX2,pcMIP,ppvZPX2,ppvMIP] =
% NMSA_to_PSICOV(nmsa,'NOGAPS',0.62,1,0.005,0.0,0.0001,'QUIC','FRO','RHO',20);
% To reproduce PSICOV use: [pcZPX2,pcMIP,ppvZPX2,ppvMIP] =
% NMSA_to_PSICOV(nmsa,'NOGAPS',0.62,1,0.015,0.0,0.0001,'GLASSO','FRO','INVERSE',20);
...
dist_threshold = false(nrows,nrows);

for i = 1:nrows
dist_threshold(:,i) = dist(:,i) >= floor(threshold*dist(i,i));
end
