function [pcZPX2,pcMIP,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_PSICOV_2(nmsa,threshold,psc_lambda,...
    Lmat,lambda,delta,inverse_method,pc_method,nsymbols)
% This function is the 2nd of three alternative implementations of the PSICOV
% algorithm. The MSA is first converted to its binary 'long' representation
...
% between sequences. A pseudocount (psc_lambda) is also applied. It is at
% this point that it differs from _1, as also the columns that contains
% all zeros are included in the pseudocount.  Then, the covariance matrix
% is calculated and is shrunk to the equivalent covariance matrix for the
...
% value of Lmat is 0.02. Suitable ranges are between 0.015 and 0.025. Lmat
...
% matrix. It affects only the sign of the MIP matrix, but not that of the
% ZPX2 matrix. Recommended usage: [pcZPX2,pcMIP] =
