function [ZPX2,md3_ZPX2,md4_ZPX2,lgZPX2,md3_lgZPX2,md4_lgZPX2,...
    ...
    NMSA_to_p4D_ZPX2_4(nmsa,dist_method,aver_method,threshold,psc_lambda,...
...
% sequences in the alignment. 'gW_e1' is the exponent of the weights on the
% 1st round of gap weighting before the MIP matrix is calculated: 0 gives a
...
% alignment.
% [ZPX2,md3_ZPX2,md4_ZPX2] = NMSA_to_4D_ZPX2(nmsa,'GAPS',0.9,1,21,0,0,3);
%
...
% [ZPX2,md3_ZPX2,md4_ZPX2] = NMSA_to_4D_ZPX2(nmsa,'GAPS',0.9,1,22,0,1,3);
%
%--------------------------------------------------------------------------
...
% [MI,md3_MI,Hij,Hijk] = NMSA_to_MI_3d(nmsa,ncols);
% [Hijkl] = NMSA_to_Hijkl(nmsa,ncols);

[MI,md3_MI,md4_MI] = NMSA_to_MI_4d(nmsa,ncols,aver_method);

% switch aver_method    
%     case 'FAST'    
%     [md4_MI] = NMSA_to_MI_4d_1(ncols,Hij,Hijk,Hijkl);
%     % md4_MI2 = md4_MI;
%     case 'BEST'
%     [md4_MI] = NMSA_to_MI_4d_2(ncols,Hij,Hijk,Hijkl);
    % md4_MI1 = md4_MI2;    
%     case 'BOTH'
%     [md4_MI1] = NMSA_to_MI_4d_1(ncols,Hij,Hijk,Hijkl);
%     [md4_MI2] = NMSA_to_MI_4d_2(ncols,Hij,Hijk,Hijkl);
...
function [MI,md3_MI,md4_MI] = NMSA_to_MI_4d(nmsa,ncols,aver_method)
%--------------------------------------------------------------------------
...
fprintf('Hijk section completed in %9.4f minutes. \n', time/60);
...
fprintf('Hijkl section started. \n');

% total_count = (ncols^4 + ncols^3 + 2*ncols^2 + ncols)/24; % Approximate count.
total_count = (ncols^4 + ncols^3 + 10*ncols^2)/24; % Approximate count.
count = 0;
time = 0;
    ...
    for j = i:ncols
        ...
        parfor k = j:ncols
            ...
            for l = k:ncols
                ...
                % Hijkl(i,j,k,l) = const - sum(ijkl_marg.*log2(ijkl_marg))/l_Meff;
...
fprintf('md4_MI section started with the FAST method. \n');
...
md4_MI_time = toc;
fprintf('md4_MI section completed in %9.4f minutes. \n', (md4_MI_time)/60);

%--------------------------------------------------------------------------

    case 'BEST'

fprintf('md4_MI section started with the BEST method. \n');
...
for i = 1:ncols
    parfor j = i:ncols
        k_ind = setdiff(all_ind,[i j]);
        k_ind_length = length(k_ind);
        
        for lk = 1:k_ind_length
            k = k_ind(lk);
            l_ind = setdiff(all_ind,[i j k]);
            
            Hij_m1 = zeros(k_ind_length,1);            
            Hijk_m2 = zeros(k_ind_length,1);            
            Hijk_m3 = zeros(k_ind_length,1);            
            Hijkl_m4 = zeros(k_ind_length,1);            
            ...
            Hij_m1(lk) = nanmean(Hij(k,l_ind),2);            
            Hijk_m2(lk) = nanmean(Hijk(i,k,l_ind),3);            
            Hijk_m3(lk) = nanmean(Hijk(j,k,l_ind),3);            
            Hijkl_m4(lk) = nanmean(Hijkl(i,j,k,l_ind),4);            
