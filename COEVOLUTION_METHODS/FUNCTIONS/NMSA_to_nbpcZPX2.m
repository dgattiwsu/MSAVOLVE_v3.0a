function [ZPX2,MIP,MI] = NMSA_to_nbpcZPX2(nmsa,Lmat)
% This function combines the traditional nb calculation with the pc
% calculation, by deriving the sparse inverse of the nbMI matrix. 
...
z_ordered(~bin_ordered) = 0;
    ...
    [mi_inv mi_rev opt cputime iter dGap] = QUIC('default', MI, Lmat, 1e-6, 2, 200);
    
    rho = zeros(ncols,ncols);
    for i = 1:ncols
        for j = i:ncols
            rho(i,j) = -mi_inv(i,j)/sqrt(mi_inv(i,i)*mi_inv(j,j));
            rho(j,i) = rho(i,j);
        end
        % rho(i,i) = NaN;
    end
