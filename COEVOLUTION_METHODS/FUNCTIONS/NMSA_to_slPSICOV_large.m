function [pcZPX2,lgZPX2,sparsity_value,pcMIP,lgMIP,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_slPSICOV_large(nmsa,dist_method,threshold,psc_lambda,nsymbols,...
    slice_scale,col_scale,norm_type,...
    Lmat,target_sparsity,delta_sparsity,...
...
% non-gaps rows. 'cov_method' determine the way the cross-covariance
% between slices is calculated: it can be set to 'CROSS' (strict 
% cross-covariance between the two slices) or 'FULL' in which case the two
% slices are concatenated to a nseqx40 or nseqx41 larger matrix, and then 
...
% taking the norm (norm_type = 1|2|'fro'|'L1') of each cross-covariance
% matrix ('1' norm is the largest value that the sum of the elements of any
% column can take; '2' norm is the largest singular value of the matrix;
% 'frobenius' norm is the 2-norm of the linearized matrix;L1 is the sum of
% the absolute values of the linearized matrix). At this point we can
% condition (cond_method = 'COND') or not condition (cond_method =
% 'NOCOND') the covariance matrix in order to make it positive definite.
% Finally, the 'QUIC' method is used to invert the covariance matrix. Lmat
...
% number of QUIK iteration allowed to reach the target sparsity is set with
...
% must be small:
% [slPSICOV,sl_lgPSICOV] = NMSA_to_slPSICOV(nmsa,'NOGAPS',0.90,20,...
% 'SUM','NONE','fro','COND',0.0001,0.3,0.015,10,0.0,0.0001,'RHO',0,1);
% Possible syntax if used with scale = 'NONE'; IMPORTANT: in this case Lmat 
% must be large:
% [slPSICOV,sl_lgPSICOV] = NMSA_to_slPSICOV(nmsa,'NOGAPS',0.90,20,...
% 'NONE','NONE','fro','COND',0.4,0.3,0.015,10,0.0,0.0001,'RHO',0,1);
...
% Here we calculate all cross-covariance matrices slice by slice and we add 
% them up to form a large covariance matrix.

% COV = zeros(ncols,ncols);

        COV = zeros(ncols*nsymbols,ncols*nsymbols);
            for i = 1:ncols
               rs = nsymbols*(i-1) + 1;
               re = rs + nsymbols -1;
               ...
               for j = i:ncols
                    ...
                    cross_cov = slice_1'*slice_2;
                    COV(rs:re,cs:ce) = cross_cov;
                    COV(cs:ce,rs:re) = cross_cov;
                end
            end    
%     uCOV = triu(COV,0);
%     lCOV = tril(COV',-1);
...
%             iter_count_fract = 1 - (iter_count/max_iter);
    ...
    sparsity_value = (nnz(cov_inv)-ncols*nsymbols)/(ncols*nsymbols*(ncols*nsymbols-1));    % number of nonzero
...
%     end
%     
    % Here we recover the norm of the inverse of the original 
    % cross-covariances.
    
            r_cov_inv = zeros(ncols,ncols);
            for i = 1:ncols
               rs = nsymbols*(i-1) + 1;
               re = rs + nsymbols -1;
               for j = i:ncols
                    cs = nsymbols*(j-1) + 1;
                    ce = cs + nsymbols -1;
                    sub_cov = cov_inv(rs:re,cs:ce);
                    r_cov_inv(i,j) = sum(abs(sub_cov(:)));
                    r_cov_inv(j,i) = r_cov_inv(i,j);
                end
            end    
