function [ZPX2,md3_ZPX2,md4_ZPX2,lgZPX2,md3_lgZPX2,md4_lgZPX2,...
    ...
    NMSA_to_p4D_ZPX2(nmsa,dist_method,threshold,psc_lambda,...
...
const = log2(l_Meff);
    ...
    [MI,md3_MI,md4_MI] = NMSA_to_MI_4d(nmsa,ncols);
...
function [MI,md3_MI,md4_MI] = NMSA_to_MI_4d(nmsa,ncols)
%--------------------------------------------------------------------------
% MI, md3_MI, md4_MI calculation.
...
for i = 1:ncols
    for j = i:ncols
        k_ind = setdiff(all_ind,[i j]);
            ...
            % The following is the MI minus the interaction information.
            % mdMI(i,j,k) = Hij(i,k) - Hijk(i,k,j) - Hi(k) + Hij(k,j);
            % Since we are averaging over the 3rd dimension, here only i
            % and j are permuted.
