function [ZPX2,md3_ZPX2,md4_ZPX2,lgZPX2,md3_lgZPX2,md4_lgZPX2,...
    ...
    NMSA_to_mdMI_nonan(nmsa,dist_method,md_method,memory_method,...
    diag_method,threshold,psc_lambda,null_symbol,gW_e1,gW_e2,gW_e3,nprocs)
...
% matrix, the 'direct coupling' md3_ZPX2, in which the the 'interaction
% information' between each i,j,k column is subtracted from MI(i;j), and
% the 'direct coupling' md4_ZPX2, in which the the 'interaction
% information' between each i,j,k,l column is subtracted from MI(i;j). The
% final ZPX2/md3_ZPX2/md4_ZPX2 maps can be further processed using a
% logistic fit PSICOV style to produce the lgZPX2/md3_lgZPX2/md4_lgZPX2
% matrices. The similarity between sequences can be determined including
% gaps ('dist_method'= 'GAPS') or excluding gaps ('dist_method'= 'NOGAPS'),
% on the basis of a 'threshold' value. For example, a threshold = 0.9 means
% sequences more than 90% identical are weighted down as if they were
% effectively 100% identical. In general, including gaps gives better
...
% scaled count of equivalent sequences in the alignment. 'md_method' can be
% '2D' (only traditional MI),'3D' (only 2D and 3D maps are calculated), or
% '4D' (2D, 3D and 4D maps are calculated). 'memory_method' determines
% whether a full 4D matrix ('FULL') is calculated to store the 4th
% dimension or whether the 4th dimension is calculate piecewise in 3D block
% ('SAVE') or 2D slices ('DEEP_SAVE'). The options 'SAVE' and 'DEEP_SAVE'
% are extremely useful for computers with limited amount of memory or for
% very long sequences: however, the calculation is slower with 'SAVE' and
% significantly slower with 'DEEP_SAVE'. 'diag_method' can be 'DIAG', in
% which case md_matrices diagonals are kept, or 'NODIAG', in which case
% md_matrices diagonals are converted to NaN's. The option 'DIAG' is only 
% necessary to carry out cluster identification by PCA. Three rounds of gap
% weighting are possible: 'gW_e1' is the exponent of the weights on the 1st
% round of gap weighting before the MIP matrix is calculated: 0 gives a
% weight of 1 to everything; 1 leaves the weight as calculated by the
...
% processors. If nprocs is set = 1 (recommended for the '2D' option) all
% parallel loops are treated as non-parallel.
%
% Possible syntax (to be used with either 3D or 4D):
%
...
%       NMSA_to_mdMI(nmsa,'GAPS','3D','FULL','NODIAG',0.9,1,21,0,0,3,12);
%
...
% [ZPX2,md3_ZPX2,md4_ZPX2] = ...
%       NMSA_to_mdMI(nmsa,'GAPS','4D','FULL','NODIAG',0.9,1,22,0,1,3,12);
%
%--------------------------------------------------------------------------
...
global W loq loq2 loq3 loq4 l_Meff const symbol diag
symbol = null_symbol;
diag = diag_method;
...
% Notice that the two loops above are completely equivalent to the much
% more intuitive average over the 3rd and 4th dimensions calculated
% column-by-column as shown below. However, since the syntax above is
% vectorized, it is much faster.
...
%             % only i and j are permuted.
%             
%             md4_MI_3 = NaN(ncols,1);
%             for k = k_ind
%                 l_ind = setdiff(all_ind,[i j k]);
%                 
%                 md4_MI_4 = NaN(ncols,1);
%                 for l = l_ind                    
%                     md4_MI_4(l) = - Hij(k,l) + Hijk(i,k,l) + ...
%                         Hijk(j,k,l) - Hijkl(i,j,k,l);
%                 end
%                 md4_MI_3(k) = nanmean(md4_MI_4);
%                 
%             end
%             
%             md4_MI(i,j) = nanmean(md4_MI_3);
%                                             
%     end
% end
...
global W loq4 l_Meff const symbol diag
...
%
% function [ MI_mat ] = NMSA_to_fastMI( nmsa,ncols )
% %--------------------------------------------------------------------------
% % This function produce a MI matrix starting from a MSA in matlab numeric
% % format (nmsa). The function uses  the "fastMI" algorithm. This function
% % is not accurate if used if null_symbol < 22.
% 
% global symbol W
% 
...
% 
% 
% function MI = fastMI(ij_col,ij_W)
% %--------------------------------------------------------------------------
...
% % observed pairs.
% 
% global loq2 l_Meff const
% 
% h = accumarray(ij_col,ij_W); 
% xy_prod = sum(h,2)*sum(h,1);
% xy_ind = h~=0;
% h(xy_ind) = h(xy_ind) + loq2;  % pseudocount added
% MI = const + (sum(h(xy_ind) .* log2(h(xy_ind)./xy_prod(xy_ind))))/l_Meff;
% 
% end
% 
%
