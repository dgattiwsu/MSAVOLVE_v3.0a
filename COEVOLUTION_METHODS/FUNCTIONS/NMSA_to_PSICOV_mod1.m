function [pcZPX2,pcMIP,ppvZPX2,ppvMIP,gCOV,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_PSICOV(nmsa,threshold,psc_lambda,...
    Lmat,lambda,delta,inverse_method,recov_method,pc_method,mip_method,nsymbols)
% This function is the 1st of three alternative implementations of the
% PSICOV algorithm. It is identical to NMSA_to_PSICOV_1 and is slightly
% faster than _2, but gives more or less the same result. It calculates a
% partial correlation matrix for the columns of an MSA. The MSA is first
% converted to its binary 'long' representation in which every column is
% extended into 21 different columns, each one representing a different aa.
% A threshold is applied for the similarity between sequences. A
% pseudocount (psc_lambda) is also applied. Then, the covariance matrix is
% calculated and is shrunk to the equivalent covariance matrix for the
% 'short' standard representation. The value of nsymbols determines whether
% we include or not gaps in the calculation of the 'shrunk' covariance
% matrix (nsymbols = 21 includes gaps; nsymbols = 20 excludes gaps).
% Finally, an inverse method is used to invert the covariance matrix. Three
% option are available for this purpose: standard 'INVERSE', 'GLASSO', or
% 'QUIC'. Lmat is a value between 1 and 0 controlling the 'sparseness' of
% the concentration matrix (smaller values produce a less sparse inverse,
% and take more time. Larger values increase the sparseness, with the
% inverse ultimately being populated only in the diagonal). The default
% value of Lmat is 0.02. Suitable ranges are between 0.025 and 0.005. Lmat
% can also be a regularization matrix of the same dimensions as the
% covariance matrix to be inverted. 'pc_method' determines whether we
% calculate the MIP matrix from the inverse covariance or from the RHO
% matrix. It appears to affects only the sign of the MIP matrix, but not
% that of the ZPX2 matrix.'mip_method' determines whether we use the
% standard MIP ('STANDARD') or a correction PSICOV style ('WEIGHTED'). The 
% ppvZPX2 and ppvMIP matrices include the correction produced by the
% application of a logistic fit to the MIP data.
% Possible usage: [pcZPX2,pcMIP,ppvZPX2,ppvMIP] =
% NMSA_to_PSICOV(nmsa,0.62,1,0.005,0.0,0.0001,'QUIC','RHO','STANDARD',20);
% To reproduce PSICOV use: [pcZPX2,pcMIP,ppvZPX2,ppvMIP] =
% NMSA_to_PSICOV(nmsa,0.62,1,0.015,0.0,0.0001,'GLASSO','INVERSE','WEIGHTED',20);
    ...
    switch pc_method
        ...
        case 'INVERSE'
            switch mip_method
                ...
                case 'WEIGHTED'
                pcMIP = MI_to_MIP2(-n_cov_inv,ncols);
            end
    end
...
function [pMIP] = MI_to_MIP2(pMI,ncols)
%--------------------------------------------------------------------------
% MIP calculation with weighting PSICOV style

mean_mat = nansum(pMI(:))*ncols*(ncols-1)*0.5;
sum_row = zeros(ncols,1);
MCA_mat = zeros(ncols,ncols);

% Here  we calculate the MCA matrix.

for m = 1:ncols
    sum_row(m) = nansum(pMI(m,:));
end

for m = 1:ncols
    for n = m:ncols
    MCA_mat(m,n)=((sum_row(m)*sum_row(n))/sqrt(ncols-1))/mean_mat;
    MCA_mat(n,m) = MCA_mat(m,n);    
    end
MCA_mat(m,m) = NaN;
end
