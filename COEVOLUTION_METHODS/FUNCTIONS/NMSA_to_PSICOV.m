function [pcZPX2,pcMIP,ppvZPX2,ppvMIP,gCOV,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_PSICOV(nmsa,dist_method,threshold,psc_lambda,cov_diag,recov_method,...
    Lmat,lambda,delta,inverse_method,pc_method,nsymbols)
% Matlab implementations of the PSICOV algorithm. It
...
% 'GAPS') different columns, each one representing a different aa. The choice
% of the distance method affects also the inclusion ('GAPS') or exclusion
% ('NOGAPS') of gaps in the calculation of the similarity between sequences. 
% A 'threshold' is applied for the similarity between sequences. A pseudocount
...
% 20 excludes gaps). However, if dist_method = 'NOGAPS' is selected, nsymbols 
% is automatically set to 20. Finally, an inverse method is used to invert the
...
% dimensions as the covariance matrix to be inverted. 'lambda' and 'delta' 
% define the conditioning of the covariance matrix prior to the calculation 
