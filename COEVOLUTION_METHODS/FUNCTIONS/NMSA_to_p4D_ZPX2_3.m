function [ZPX2,md3_ZPX2,md4_ZPX2,lgZPX2,md3_lgZPX2,md4_lgZPX2,...
    ...
    NMSA_to_p4D_ZPX2_3(nmsa,dist_method,md_method,aver_method,threshold,...
...
% calculated), or '4D' (both 3D and 4D maps are calculated). 'aver_method'
% determines the method used to average the 3rd and 4th dimension: it can
% be 'SINGLE' (single processor), or 'PARALLEL' (parallel processing); if
% the available memory is exceeded, it usually helps to switch to 'SINGLE'.
% 'gW_e1' is the exponent of the weights on the 1st round of gap weighting
% before the MIP matrix is calculated: 0 gives a weight of 1 to everything;
% 1 leaves the weight as calculated by the 'correct_coevmat_forgaps'
% function; powers between 0 and 1 increase the weight, powers > 1 make the
% weight progressively smaller. Additional 2 rounds of gap weighting are
% possible after the MIP matrix is calculated ('gW_e2'), and after the ZPX2
% matrix is calculated ('gW_e3'). 'null_symbol' is a numeric value (between
...
% alignment.
% [ZPX2,md3_ZPX2,md4_ZPX2] = NMSA_to_4D_ZPX2(nmsa,'GAPS','4D',0.9,1,21,0,0,3);
%
...
switch md_method
    ...
    switch aver_method    
        case 'SINGLE'    
            [md4_MI] = NMSA_to_4D_MI(ncols,Hij,Hijk,Hijkl);
        case 'PARALLEL'
...
fprintf('Hijkl/MI_4d section started. \n');

total_count = (ncols^4 + ncols^3 + ncols^2 + ncols)/24; % Approximate count.
count = 0;
time = 0;
