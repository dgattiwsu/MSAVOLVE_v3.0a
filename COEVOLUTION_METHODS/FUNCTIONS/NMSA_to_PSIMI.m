function [pcZPX2,pcMIP,ppvZPX2,ppvMIP,gCOV,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_PSIMI(nmsa,long_method,cov_diag,recov_method,...
    Lmat,lambda,delta,inverse_method,pc_method,nsymbols)
% Matlab experimental implementations of the PSIMI algorithm. It
% calculates a MI matrix for the columns of an MSA. The
...
% 'GAPS) different columns, each one representing a different aa. Then, the 
% MI matrix is calculated and is converted to the equivalent MI matrix for 
% the 'short' standard representation. Before this conversion it is possible  keep
% to (cov_diag = 'KEEP') or zero (cov_diag = 'ZERO') [recommended] the
% diagonal of the large MI matrix. The conversion to the 'short'
...
% covariance matrix. Two option are available for this purpose: standard
% 'INVERSE', 'QUIC'. Lmat is a value between 1 and 0
...
% MI matrix
ogCOV = BNMSA_to_MI(s_bin_ordered,nrows,n_bin_ordered_ind);
gCOV(bin_ordered_ind,bin_ordered_ind) = ogCOV;

% Covariance matrix
% ogCOV = cov(s_bin_ordered,nrows,n_bin_ordered_ind);
% gCOV(bin_ordered_ind,bin_ordered_ind) = ogCOV;
