function [pcZPX2,lgZPX2,sparsity_value,pcMIP,lgMIP,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_slPSICOV_fro(nmsa,dist_method,threshold,psc_lambda,nsymbols,...
...
COV = zeros(ncols,ncols);
        ...
        if strcmp(norm_type,'L1')
            ...
            for i = 1:ncols
                ...
                for j = i:ncols
                    slice_2 = squeeze(nmsa_3(:,j,:));
                    c_slice = slice_1'*slice_2;
                    m1_slice = mean(c_slice,1);
                    m2_slice = mean(c_slice,2);
                    m_slice = mean(m2_slice);
                    
                    % linearly transformed cross-slice Weigt style 
                    for a = 1:nsymbols
                        for b = 1:nsymbols
                            c_slice(a,b) = ...
                                c_slice(a,b) - m1_slice(a) - m2_slice(b) ...
                                + m_slice;
                        end
                    end
                    
                    COV(i,j) = norm(c_slice,norm_type);
                    COV(j,i) = COV(i,j);
                end
            end
        end
