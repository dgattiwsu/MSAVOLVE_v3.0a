function [ZPX2,md3_ZPX2,md4_ZPX2,lgZPX2,md3_lgZPX2,md4_lgZPX2,...
    ...
    NMSA_to_p4D_ZPX2_2(nmsa,dist_method,threshold,psc_lambda,...
...
[md4_MI] = NMSA_to_MI_4d(nmsa,ncols,Hij,Hijk);
...
function [MI,md3_MI,Hij,Hijk] = NMSA_to_MI_3d(nmsa,ncols)
%--------------------------------------------------------------------------
...
function [md4_MI] = NMSA_to_MI_4d(nmsa,ncols,Hij,Hijk)
%--------------------------------------------------------------------------
% md4_MI calculation.
...
for i = 1:ncols    
    ...
    for j = i:ncols
        ...
        for k = j:ncols
            ...
            for l = k:ncols
                ...
                ijkl_col = nmsa(:,[i j k l]);
                ijkl_ind = any(ijkl_col == symbol,2);
                ijkl_col = ijkl_col(~ijkl_ind,:);
                ijkl_W = W(~ijkl_ind);

                h = accumarray(ijkl_col, ijkl_W);

                ijkl_marg = nonzeros(h) + loq4;      % pseudocount
    ...
    time_left = count_left*(time/count)/60;
    fprintf('Time elapsed: %6.1f minutes \n',time/60);            
    fprintf('Est. time to Hijkl/MI_4d completion: %6.1f minutes \n',time_left);
