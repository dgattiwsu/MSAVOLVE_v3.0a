function [pcZPX2,pcMIP,ppvZPX2,ppvMIP,gCOV,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_PSICOV_1(nmsa,threshold,psc_lambda,...
...
% that of the ZPX2 matrix. The ppvZPX2 and ppvMIP matrices include the
% correction produced by the application of a logistic fit to the MIP data.
...
ndata = length(data);
all_ind = 1:ndata;
nobs = sum(isfinite(data));

data_sum = nansum(data);
data_mean = nanmean(data);

% Traditional
data_std = nanstd(data);
% Psicov style ... not clear to me.
% data_std = sqrt(data_sum^2/nobs - data_mean^2); 
% Logistic
