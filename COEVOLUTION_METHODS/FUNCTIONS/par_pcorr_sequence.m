function [pcorr_nulsp,W,pcorr_colsp,pcorr_vsum,pcorr_orig] = ...
    par_pcorr_sequence(nmsa,dist_method,threshold,psc_lambda,nsymbols,...
    slice_scale,col_scale,proj_space,mat_norm,nprocs)
...
% 'scale' determines whether the columns are divided by the number of
...
switch proj_space
    ...
    case 'NULL'
        ...
        for i = 1:ncols_o
            ...
            i_block = (i-1)*nsymbols+1:(i-1)*nsymbols+nsymbols;
            i_slice = nmsa(:,i_block);
                       
           parfor j = i:ncols_o
                
                j_block = (j-1)*nsymbols+1:(j-1)*nsymbols+nsymbols;
                j_slice = nmsa(:,j_block);
        
                % Here we select all the columns except the two for which
                % we want to measure the correlation coefficient.
                               
                row1 = row;
                row1([i_block j_block]) = false;
                s_nmsa = nmsa(:,row1);
...
%                 end
                ...
                pcorr_nulsp(i,j) = norm(i_slice'*P2*j_slice,mat_norm);
                % pcorr_nulsp(j,i) = pcorr_nulsp(i,j);
