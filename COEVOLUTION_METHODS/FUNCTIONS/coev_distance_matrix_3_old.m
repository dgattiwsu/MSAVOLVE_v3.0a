...
% SEQRES field of the header. We suggest that the 'SEQRES' value should be
% equal to the residue number of the last aa for which there are
% coordinates. It doesn't matter if there are initial or central gaps in
% the actual coordinates, because these will correspond to all zero values
% in the distance matrix. Therefore if you have leading or trailing
% residues (e.g. a HIS-tag) that were not included in the msa and therefore
% in the coevolution matrix, it is crucial to specify correctly what are
% the 1st and last residue numbers in the coordinates, which correspond to
% the msa for which there is a coevolution matrix. If the coordinates start
% at a residue that comes after the beginning of the msa it is better to
% trim the msa in such a way that there is full correspondance between 1st
% and last residues in the msa and in the pdb.
...
%--------------------------------------------------------------------------
plot_dist_map(sorted_mat1,X,Y,npreds,CM,S0,ps,pc,mult,shift_res_no);
% freezeColors(gca);
...
%--------------------------------------------------------------------------
plot_dist_map(sorted_mat2,X,Y,npreds,CM,S0,ps,pc,mult,shift_res_no);
% freezeColors(gca);
...
%--------------------------------------------------------------------------
plot_dist_map(sorted_mat3,X,Y,npreds,CM,S0,ps,pc,mult,shift_res_no);
% freezeColors(gca);
...
%--------------------------------------------------------------------------
plot_dist_map(sorted_mat4,X,Y,npreds,CM,S0,ps,pc,mult,shift_res_no);
% freezeColors(gca);
