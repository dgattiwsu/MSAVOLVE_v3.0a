...
% taking the norm (norm_type = |2|'fro'|) of each cross-covariance matrix
% ('2' norm is the largest singular value of the matrix; 'frobenius' norm
% is the 2-norm of the linearized matrix). Finally, an inverse method is
% used to invert the covariance matrix. Three option are available for this
% purpose: standard 'INVERSE', 'GLASSO', or 'QUIC'. Lmat is a value between
...
% between 0.025 and 0.0001. Lmat can also be a regularization matrix of the
% same dimensions as the covariance matrix to be inverted. 'lambda' and
...
% then we merge the two matrices. The ppvZPX2 and ppvMIP matrices include
% the correction produced by the application of a logistic fit to the MIP
% data. Best results are usually achieved when the inverse sparsity is
% around 0.2.
%
% Possible syntax:
% [pcZPX2,~,ppvZPX2,~] = NMSA_to_slicePSICOV(nmsa,'NOGAPS',0.90,20,'fro',...
%    0.002,0.0,0.0001,'QUIC','RHO');
...
% nmsa and represents one of 20 symbols (if gaps are excluded). 
...
% center all the vectors. Then we calculate all the dot products and
% extract the norm.
...
for i = 1:ncols
    ...
    for j = i:ncols
        ...
        m_slice_1 = mean(slice_1);
        m_slice_2 = mean(slice_2);
        m_slice_1 = m_slice_1(ones(nrows,1),:);
        m_slice_2 = m_slice_2(ones(nrows,1),:);
        c_slice_1 = slice_1 - m_slice_1;
        c_slice_2 = slice_2 - m_slice_2;
        
        COV(i,j) = norm((c_slice_1'*c_slice_2)/nrows,norm_type);
        COV(j,i) = COV(i,j);
    end
end
