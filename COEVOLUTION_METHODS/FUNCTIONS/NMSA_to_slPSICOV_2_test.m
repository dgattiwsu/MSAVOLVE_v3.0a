function [pcZPX2,sparsity_value,pcMIP,ppvZPX2,ppvMIP,cov_inv,rho,COV,sCOV,lambda] = ...
    NMSA_to_slPSICOV_2(nmsa,dist_method,threshold,psc_lambda,nsymbols,...
...
% 'frobenius' norm is the 2-norm of the linearized matrix). Finally, the
% 'QUIC' method is used to invert the covariance matrix. Lmat is a value
% between 1 and 0 controlling the 'sparseness' of the concentration matrix
...
% Suitable ranges are between 0.025 and 0.0001. Lmat can also be a
...
% values appear to be 0.3 and 0.015. The maximum number of QUIK iteration
...
% calculation of the inverse faster, but possibly less accurate.
% 'pc_method' = RHO|INVERSE|BOTH determines whether we calculate the MIP
% matrix from the inverse covariance, from the RHO matrix, or from both and
% then we merge the two matrices. 'gW_e1' is the exponent of the gap
% weights applied before the ZPX2 matrix is calculated: 0 gives a weight of
% 1 to everything; 1 leaves the weight as calculated by the
...
% exponent of the weights computed after the ZPX2 matrix.The ppvZPX2 and
...
% optimal result.
%
% Possible syntax if used with scale = 'SUM'; IMPORTANT: in this case Lmat 
% must be small:
% [slPSICOV] = NMSA_to_slPSICOV(nmsa,'NOGAPS',0.90,20,...
% 'SUM','NONE','fro',0.0001,0.3,0.015,10,0.0,0.0001,'RHO',0,1);
% Possible syntax if used with scale = 'NONE'; IMPORTANT: in this case Lmat 
% must be large:
% [slPSICOV] = NMSA_to_slPSICOV(nmsa,'NOGAPS',0.90,20,...
% 'NONE','NONE','fro',0.4,0.3,0.015,10,0.0,0.0001,'RHO',0,1);
...
% Here we test for positive definitiveness
    ...
    elseif sparsity_value < sparsity_range(1)
        if sparsity_value <= 1e-6
            ...
            % to the target sparsity before we reach the maximum iteration
            % value.
            Lmat = 0.9*Lmat;
        else
