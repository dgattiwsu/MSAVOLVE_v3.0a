function [ZPX2,MI_ref,COV_ref,MI_sig,COV_sig] = ...
    NMSA_to_db4_ZPX2(nmsa,nshuffle,...
    dist_method,sort_method,gaps_method,merge_method)
% This function differs from 'db2' by the fact that it calculates the 
% significance of the db matrix based on the 'null' evolutionary hypothesis 
% (no correlation between columns). The null hypothesis is tested by 
% shufling 'nshuffle' times every column of the msa, and calculating the db 
% matrix for each shuffled msa. Then, each shuffled db matrix is tested to 
% see if its values are higher or lower than the unshuffled matrix.
% 'merge_method' can be 'SCALE' (scale MI and COV matrix) or 'NOSCALE'.
%--------------------------------------------------------------------------
% The original 'db2 function expands the 'db' method by allowing a selection of the
...
% (sort_method = 'WEIGHTS'). Then, it is also possible to include 
% (gap_method = 'INCLUDE') or exclude (gap_method = 'EXCLUDE') the gaps from 
% the differential binary matrix. The default synthax for db2, which will give 
% a result essentially identical to NMSA_dbZPX2 is: 
% [dbZPX2] = NMSA_to_db2_ZPX2(nmsa,'GAPS','DISTANCE','INCLUDE');
%--------------------------------------------------------------------------
% The recommended syntax for db4 uses a value of at least 100 for 'nshuffle':
% [db4_ZPX2] = NMSA_to_db4_ZPX2(nmsa,100,'GAPS','DISTANCE','INCLUDE','SCALE')
...
MI_sig_plus = zeros(ncols,ncols);
MI_sig = zeros(ncols,ncols);
COV_sig_plus = zeros(ncols,ncols);
COV_sig = zeros(ncols,ncols);
...
for i = 1:nshuffle
    ...
    [MI_shuf,COV_shuf] = NMSA_to_MI_COV(shuffled_nmsa,...
        dist_method,sort_method,...
        gaps_method,nrows,ncols);
    MI_sig_plus = MI_ref >= MI_shuf;
    COV_sig_plus = COV_ref >= COV_shuf;
    MI_sig = MI_sig + MI_sig_plus;
    COV_sig = COV_sig + COV_sig_plus;
end

    MI_sig = MI_sig/nshuffle;
    COV_sig = COV_sig/nshuffle;
           
    MI = (MI_ref .* MI_sig);
    COV = (COV_ref .* COV_sig);

% Here we merge MI and covariance matrix. 
