function [ZPX2,md3_ZPX2,md4_ZPX2,lgZPX2,md3_lgZPX2,md4_lgZPX2,...
    ...
    NMSA_to_mdMI_sm(nmsa,dist_method,md_method,memory_method,threshold,...
...
% sequences in the alignment. 'md_method' can be '2D' (only traditional MI),
%'3D' (only 2D and 3D maps are
% calculated), or '4D' (2D, 3D and 4D maps are calculated).
...
% 'DEEP_SAVE'. Three rounds of gap weighting are possible: 'gW_e1' is the exponent of the weights on the 1st round of
...
% three values to 0 eliminates all gap weights.
...
switch md_method
    ...
    case '3D'
        [MI,Hi,Hij] = NMSA_to_2D_MI(nmsa,ncols);        
        [md3_MI] = NMSA_to_3D_MI(nmsa,ncols,Hi,Hij);
        md4_MI = md3_MI;
        
    case '4D'
        md4_ts = tic;
        [MI,Hi,Hij] = NMSA_to_2D_MI(nmsa,ncols);        
        [md3_MI,Hijk] = NMSA_to_3D_MI(nmsa,ncols,Hi,Hij);
...
function [MI,Hi,Hij] = NMSA_to_2D_MI(nmsa,ncols)
%--------------------------------------------------------------------------
% MI calculation.
...
function [md3_MI,Hijk] = NMSA_to_3D_MI(nmsa,ncols,Hi,Hij)
%--------------------------------------------------------------------------
% MI_3d calculation.

global W loq3 l_Meff const symbol
