function [ZPX2,md3_ZPX2,md4_ZPX2,lgZPX2,md3_lgZPX2,md4_lgZPX2,...
    ...
    NMSA_to_mdMI(nmsa,dist_method,md_method,aver_method,threshold,...
...
for i = 1:ncols
    ...
    parfor j = i:ncols
        % k_ind = setdiff(all_ind,[i j]);
%         k_ind_l = length(k_ind);
%         
%         % Here we only get the values of the Hij(k,l) term.
%         for lk = 1:k_ind_l
%             k = k_ind(lk);
%             l_ind = setdiff(all_ind,[i j k]);
%             Hij_m1 = zeros(k_ind_l,1);            
%             Hij_m1(lk) = nanmean(Hij(k,l_ind),2);
%         end

            % Here we reset l_ind as the same as k_ind
            % l_ind = k_ind;
