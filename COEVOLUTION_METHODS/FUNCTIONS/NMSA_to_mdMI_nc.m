function [ZPX2,md3_ZPX2,md4_ZPX2,lgZPX2,md3_lgZPX2,md4_lgZPX2,...
    ...
    NMSA_to_mdMI_nc(nmsa,dist_method,md_method,aver_method,threshold,...
...
% (not used if 'md_method = 3D') determines the method used to average the
% 3rd and 4th dimension : it can be 'SINGLE' (single processor), or
% 'PARALLEL' (parallel processing); if the available memory is exceeded, it
% usually helps to switch to 'SINGLE'. 'gW_e1' is the exponent of the
...
% alignment.
% [ZPX2,md3_ZPX2,md4_ZPX2] = NMSA_to_4D_ZPX2(nmsa,'GAPS','4D','SINGLE',0.9,1,21,0,0,3);
%
...
% [ZPX2,md3_ZPX2,md4_ZPX2] = NMSA_to_4D_ZPX2(nmsa,'GAPS','4D','PARALLEL',0.9,1,22,0,1,3);
%
%--------------------------------------------------------------------------
...
[o_nrows,o_ncols] = size(nmsa);
sum_nmsa = sum(nmsa)/o_nrows;
cons_matrix = zeros(21,o_ncols);
for i = 1:21
...
o_nmsa = nmsa;
nmsa = nmsa(:,noncons_ind);

% Here we start processing the reduced nmsa.
...
switch md_method
    case '3D'
        [MI_s,md3_MI_s] = NMSA_to_3D_MI(nmsa,ncols);
        md4_MI_s = md3_MI_s;
        
    case '4D'
        md4_ts = tic;
        
        [MI_s,md3_MI_s,Hij,Hijk] = NMSA_to_3D_MI(nmsa,ncols);
        [Hijkl] = NMSA_to_Hijkl(nmsa,ncols);

        switch aver_method    
            case 'SINGLE'    
                [md4_MI_s] = NMSA_to_4D_MI(ncols,Hij,Hijk,Hijkl);
            case 'PARALLEL'
                [md4_MI_s] = NMSA_to_p4D_MI(ncols,Hij,Hijk,Hijkl);        
        end
...
% Here we return every array to the original size containing also the
% conserved columns.
...
md3_MI = zeros(ncols,ncols);
md4_MI = zeros(ncols,ncols);
MI(noncons_ind,noncons_ind) = MI_s;
md3_MI(noncons_ind,noncons_ind) = md3_MI_s;
md4_MI(noncons_ind,noncons_ind) = md4_MI_s;
clear MI_s md3_MI_s md4_MIs
...
for i = 1:ncols
    ...
    parfor j = i:ncols
            ...
            Hij_m1 = nanmean(Hij,2);
            
            Hijk_m2 = nanmean(Hjk,2);
            
            Hijk_m3 = nanmean(Hijk(j,:,:),3);
            
            Hijkl_m4 = nanmean(Hjkl(j,:,:),3);
