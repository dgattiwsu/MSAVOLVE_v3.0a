function [ evolved_MSA,history,mut_history,cov_history,...
    ...
    evolve_6s_3D_cov_hist_for_testing( MSA,history,mut_history,cov_history,...
...
% 1st pass: we find the first occurrence of a covarion member in the 
% logical vector of positions flagged for mutation.
 for i = 1:size(covar_vec,1)
    ind = covar_vec(i,:);
    for j = 1:nseq
        if (msa_mut_pos_mat(j,ind(1))||msa_mut_pos_mat(j,ind(2)))
        msa_cov_pos_mat(j,ind) = true;
        end
    end
 end
% 2nd pass: we find all the other occurrences of a member of a pair in
% other pairs.
 for i = 1:size(covar_vec,1)
    ind = covar_vec(i,:);
    for j = 1:nseq
        temp_cov_vec = + msa_cov_pos_mat(j,:);
        if any(intersect(ind,temp_cov_vec))
        msa_cov_pos_mat(j,ind) = true;
        end
    end
 end
% 3rd pass: we remove from the mutation list any position that was put in
% the covarion list.
 for j = 1:nseq
...
%--------------------------------------------------------------------------
% Option 1:
% If higher order coevolution exists (e.g. triplets,
% quartets,tec.), then the correct thing to do would be to recalculate the
% joint probability distributions for those multiplets. However, this would
% really slow down the program considerably, because with even just a
% triplet we have 20^3 configurations. Let's consider the other options.
%--------------------------------------------------------------------------        
% Option 2:
% Here we are going to scan along the sequences rather than along the
% positions. This is the standard option, but it is clearly biased in the
% case of multiplets, because every member of every pair in a multiplet
% will take the symbol derived from the joint PD of the last pair of the
% mutliplet in the covar_vec.
% MSA2 = MSA1;
% % Here we loop over the covarying positions.
% for i = 1:size(covar_vec,1)
%     % Here we loop over all the sequences in the msa
%     for j = 1:nseq
%         % Here for every sequence we check if the covarying position
%         % represented in the loop is flagged
%         if msa_cov_pos_mat(j,covar_vec(i,:))
%             % Here we get the covarying residues.
%             ind = round(random(cov_prob_PD{i}));
%             MSA2(j,covar_vec(i,1)) = cov_alphabet(ind,1,i);
%             MSA2(j,covar_vec(i,2)) = cov_alphabet(ind,2,i);
%         end
%     end
% end
%--------------------------------------------------------------------------    
