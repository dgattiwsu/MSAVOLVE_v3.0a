function [ msa_covar_vec_low,msa_covar_vec_mid,msa_covar_vec_high ] = ...
    pick_covarions_4( msa,bg_frequencies,npos,fr_cov,cov_gaps )
...
msa_nogaps = msa;
msa_nogaps(:,~msa_nogapspos) = 0;
msa = msa_nogaps;
end

smsa = int2aa(msa);
[profile,~] = seqprofile(smsa,'Gaps','none');
[~,rel_entropy] = rel_entropy_nmsa(msa,bg_frequencies,profile);
msa_entropy = rel_entropy';
