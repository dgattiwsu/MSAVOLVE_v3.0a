...
[MSA_select_x,MSA_select_y,MSA_select_z] = size(MSA_select_ALL);

MSA_cov_mat = zeros(MSA_select_x,MSA_select_x,MSA_select_z);
MSA_cov_mat_2 = MSA_cov_mat;

for i = 1:MSA_select_z
c_MSA_select = int2aa(MSA_select_ALL(:,:,n));    
 if DISTANCE == 1
 MSA_dist = seqpdist(c_MSA_select,'SquareForm',true,'Method','p-distance');
 MSA_cov_mat(:,:,i) = 1-MSA_dist;
 MSA_cov_mat_2(:,:,i) = sort_cov_mat(MSA_select_ALL(:,:,n),MSA_cov_mat(:,:,i));
 elseif DISTANCE == 2
 ...
 MSA_cov_mat(:,:,i) = (max(MSA_dist(:))-MSA_dist)/max(MSA_dist(:));
 MSA_cov_mat_2(:,:,i) = sort_cov_mat(MSA_select_ALL(:,:,n),MSA_cov_mat(:,:,i));
 else
 MSA_binmsa = nmsa_to_binmsa(MSA_select);
 MSA_cov_mat(:,:,i) = cov(MSA_binmsa',1);  
 MSA_cov_mat_2(:,:,i) = sort_cov_mat(MSA_select_ALL(:,:,n),MSA_cov_mat(:,:,i));
 end
end

 MSA_cov_mat = nanmean(MSA_cov_mat,3);
 o_MSA_cov_mat = nanmean(MSA_cov_mat_2,3);
...
hold on
stairs(nanmean(MSA_rel_entropy),'-r','DisplayName','msa_rel_entropy');
hold off
xlabel('Seq. Position','FontSize',14,'FontWeight','n'); 
ylabel('Relative Entropy','FontSize',14,'FontWeight','n');
